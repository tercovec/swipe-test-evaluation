...
def send_mail(graphs):
  def figure_tags(graphs):
    ...
    for graph, graph_description in graphs:
      ...
      tag += f'''<figure><img alt="Swipe result" style="width:95%;height:auto" src="data:image/jpeg;base64,{helpers.get_b64string(graph.get_bytes())}"><br><figcaption>{graph_description or ''}</figcaption></figure>'''
    return tag
